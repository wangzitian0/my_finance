...
def list_root_files():
    ...
    config_files = [f for f in files if any(ext in f for ext in ['.yml', '.yaml', '.toml']) and 'config' in f.lower()]
    core_files = ['README.md', 'CLAUDE.md', 'pyproject.toml', 'pixi.toml', 'p3', '.gitignore', 'MIGRATION_SUMMARY.md']
    
    other_files = [f for f in files if f not in coverage_files + test_files + config_files + core_files]
