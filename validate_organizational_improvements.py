...
def validate_inter_agent_communication():
    ...
    if communication_protocol.exists():
        ...
        has_consultation_protocol = (
            "AgentConsultation" in content and "Agent Consultation Protocol" in content
        )
        has_collaboration_protocol = (
            "CollaborativeProblem" in content
            and "Collaborative Problem-Solving Protocol" in content
        )
        has_session_continuity = (
            "SessionBridge" in content and "CONTEXT CONTINUITY MECHANISMS" in content
        )
