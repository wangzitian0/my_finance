...
class BacktestEngine:
    ...
    def run_backtest(
        self, strategy_signals: List[StrategySignal], config: BacktestConfig
    ) -> BacktestResults:
        """
        ...
        try:
            ...
            simulation_results = self._run_simulation(
                strategy_signals, portfolio, market_data, config
            )

            # Calculate performance metrics
            performance_metrics = self._calculate_performance_metrics(simulation_results, config)

            # Calculate benchmark comparison
            benchmark_metrics = self._calculate_benchmark_comparison(simulation_results, config)
            ...
            results = BacktestResults(
                **performance_metrics, **benchmark_metrics, **risk_metrics, **simulation_results
            )
    ...
    def _load_market_data(
        self, signals: List[StrategySignal], config: BacktestConfig
    ) -> pd.DataFrame:
    ...
    def _run_simulation(
        ...
        for date in market_data.index:
            ...
            portfolio_value = self._calculate_portfolio_value(portfolio, market_data.loc[date])

            portfolio_values.append(
                {"date": date, "portfolio_value": portfolio_value, "cash": portfolio["cash"]}
            )

            # Record current positions
            positions_history.append({"date": date, **portfolio["positions"].copy()})
    ...
    ) -> Dict[datetime, List[StrategySignal]]:
    ...
    def _execute_rebalancing(
        ...
        for signal in signals:
            if signal.action in ["BUY", "SELL"]:
                trade = self._execute_trade(portfolio, signal, prices, portfolio_value, config)
                if trade:
                    trades.append(trade)
    ...
    def _execute_trade(
        ...
        transaction_cost = self._calculate_transaction_costs(trade_shares, current_price, config)
    ...
    def _calculate_transaction_costs(
        self, shares: Decimal, price: Decimal, config: BacktestConfig
    ) -> Decimal:
    ...
    def _calculate_performance_metrics(
        self, simulation_results: Dict, config: BacktestConfig
    ) -> Dict:
        ...
        total_return = (
            portfolio_values["portfolio_value"].iloc[-1]
            / portfolio_values["portfolio_value"].iloc[0]
        ) - 1
        ...
        sharpe_ratio = (
            (annualized_return - risk_free_rate) / volatility if volatility > 0 else Decimal("0")
        )
    ...
    def _calculate_risk_metrics(self, simulation_results: Dict) -> Dict:
        ...
        cvar_95 = (
            abs(returns[returns <= -var_95].mean())
            if not returns[returns <= -var_95].empty
            else var_95
        )
