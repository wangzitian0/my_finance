...
class TestModuleIntegrationWorkflow:
    ...
    def test_complete_pipeline_f2(self):
        ...
        result = subprocess.run(["pixi", "run", "clean"], capture_output=True, text=True)
        # Don't assert on clean, it might fail if nothing to clean

        # 2. Build F2 dataset (should be fast)
        result = subprocess.run(["pixi", "run", "build-f2"], capture_output=True, text=True)
        ...
        assert (
            "MSFT" in dcf_content or "NVDA" in dcf_content
        ), "F2 companies not found in DCF report"
    ...
    def test_command_integration(self):
        ...
        # Test dev command
        result = subprocess.run(["pixi", "run", "dev"], capture_output=True, text=True)
        ...
        result = subprocess.run(["pixi", "run", "quick-test"], capture_output=True, text=True)
    ...
    def test_build_status_integration(self):
        ...
        result = subprocess.run(["pixi", "run", "build-status"], capture_output=True, text=True)
        ...
        if "No builds found" not in result.stdout:
            ...
            assert any(
                indicator in output for indicator in status_indicators
    ...
    def test_error_handling_integration(self):
        ...
        result = subprocess.run(
            ["pixi", "run", "build-dataset", "invalid_tier"], capture_output=True, text=True
        )
...
class TestPipelinePerformance:
    ...
    def test_command_response_time(self):
        ...
        for command in quick_commands:
            start_time = time.time()
            result = subprocess.run(["pixi", "run", command], capture_output=True, text=True)
            end_time = time.time()
