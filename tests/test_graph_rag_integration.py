...
def test_etl_data_flow_simulation():
    ...
    try:
        ...
        for ticker in MAGNIFICENT_7_CIKS.keys():
            ...
            integration_stats["node_types"]["Stock"] = (
                integration_stats["node_types"].get("Stock", 0) + 1
            )
            ...
            for i in range(2):
                ...
                integration_stats["nodes_created"] += 1
                integration_stats["node_types"]["SECFiling"] = (
                    integration_stats["node_types"].get("SECFiling", 0) + 1
                )
...
def test_dcf_engine_query_pipeline():
    ...
    try:
        ...
        for question, expected_intent in zip(test_queries, expected_intents):
            ...
            logger.info(
                f"  Intent: {structured_query.intent.value} (expected: {expected_intent.value})"
            )
