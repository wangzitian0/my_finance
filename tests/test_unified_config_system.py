...
from common.unified_config_loader import UnifiedConfigLoader, CompanyInfo
...
class TestUnifiedConfigSystem(unittest.TestCase):
    ...
    def test_backward_compatibility(self):
        ...
        for tier in [DatasetTier.F2, DatasetTier.M7, DatasetTier.N100, DatasetTier.V3K]:
            with self.subTest(tier=tier):
                ...
                legacy_tickers = set(legacy_config.expected_tickers) if legacy_config.expected_tickers else set()
    ...
    def test_vti_weight_data(self):
        ...
        weighted_companies = [
            company for company in vti_config.companies.values() 
            if company.weight is not None
        ]
