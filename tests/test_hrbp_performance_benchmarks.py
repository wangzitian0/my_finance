...
class TestHRBPCLIPerformance:
    ...
    def test_hrbp_status_response_time(self, project_root, performance_thresholds):
        ...
        try:
            ...
            assert (
                execution_time < performance_thresholds["cli_response_time"]
            ), f"HRBP status too slow: {execution_time:.2f}s > {performance_thresholds['cli_response_time']}s"

            assert (
                memory_delta < performance_thresholds["memory_usage"]
    ...
    def test_comprehensive_cli_performance(self, project_root, performance_thresholds):
        ...
        try:
            ...
            assert execution_time < 2.0, f"Comprehensive CLI too slow: {execution_time:.2f}s"
...
class TestHRBPConcurrencyPerformance:
    ...
    def test_concurrent_hrbp_status_commands(self, project_root, performance_thresholds):
        ...
        def run_hrbp_status():
            ...
            try:
                ...
                execution_time = time.time() - start_time
                return {"execution_time": execution_time, "returncode": -1, "success": False}
        ...
        assert (
            total_execution_time < performance_thresholds["concurrent_operations"]
        ), f"Concurrent operations too slow: {total_execution_time:.2f}s"

        # Memory usage should be reasonable
        assert (
            memory_delta < performance_thresholds["memory_usage"] * 2
    ...
    def test_mixed_command_performance(self, project_root, performance_thresholds):
        ...
        commands = [["hrbp-status"], ["--help"], ["hrbp-history"], ["status"]]
        ...
        def run_command(cmd):
            ...
            try:
                ...
                execution_time = time.time() - start_time
                return {"command": cmd, "execution_time": execution_time, "success": False}
        ...
        successful_runs = sum(1 for r in results if r["success"])
        assert (
            successful_runs >= len(commands) // 2
...
class TestHRBPComponentPerformance:
    ...
    def test_performance_manager_mock_analysis(self, performance_thresholds):
        ...
        with patch(
            "common.hrbp_performance_manager.get_hrbp_performance_manager"
        ) as mock_get_manager:
    ...
    def test_coordination_optimizer_mock_performance(self, performance_thresholds):
        ...
        with patch(
            "common.agent_coordination_optimizer.get_coordination_optimizer"
            ...
            mock_metrics = {
                ...
                "capacity_utilization": {f"agent-{i}": 0.7 + (i * 0.05) for i in range(5)},
...
class TestHRBPMemoryPerformance:
    ...
    def test_hrbp_command_memory_usage(self, project_root, performance_thresholds):
        ...
        def measure_command_memory(cmd):
            ...
            try:
                ...
                return {"command": cmd, "memory_delta": memory_delta, "success": False}

        commands = [["hrbp-status"], ["hrbp-history"], ["hrbp-config"]]
        ...
        for result in memory_results:
            ...
            assert (
                result["memory_delta"] < memory_threshold
    ...
    def test_hrbp_memory_leak_detection(self, project_root):
        ...
        assert (
            memory_increase < leak_threshold
    ...
    def test_hrbp_component_memory_efficiency(self):
        ...
        with (
            patch("common.hrbp_integration_framework.HRBPIntegrationFramework") as mock_framework,
            patch("common.hrbp_performance_manager.HRBPPerformanceManager") as mock_manager,
            patch(
                "common.agent_coordination_optimizer.AgentCoordinationOptimizer"
            ) as mock_optimizer,
        ):
        ...
        assert (
            memory_delta < 10.0
        ), f"Component initialization uses too much memory: {memory_delta:.1f}MB"
...
class TestHRBPScalabilityPerformance:
    ...
    def test_coordination_scalability_mock(self, performance_thresholds):
        ...
        with patch(
            ...
            large_capacity_data = {f"agent-{i:02d}": 0.6 + (i % 30) * 0.01 for i in range(50)}
            ...
            mock_coordination_metrics = {
                ...
                + len(large_capacity_data) * 2,  # Scales with agents
            }
            ...
            assert (
                metrics["coordination_overhead_ms"] < 500
...
def run_performance_benchmark_suite():
    ...
    try:
        ...
        benchmark_results.append(
            {"test": "CLI Response Time", "value": -1, "threshold": 2.0, "status": "ERROR"}
        )
    ...
    try:
        ...
        benchmark_results.append(
            {"test": "Memory Usage", "value": memory_delta, "threshold": 50.0, "status": status}
        )

    except Exception as e:
        print(f"  Memory Usage: ERROR - {e}")
        benchmark_results.append(
            {"test": "Memory Usage", "value": -1, "threshold": 50.0, "status": "ERROR"}
        )
    ...
    try:
        ...
        print(
            f"  Concurrent Operations: {execution_time:.2f}s ({successful}/3 successful) - {status}"
        )
        ...
        benchmark_results.append(
            {"test": "Concurrent Operations", "value": -1, "threshold": 5.0, "status": "ERROR"}
        )
    ...
    for result in benchmark_results:
        ...
        if result["value"] >= 0:
            print(
                f"{status_icon} {result['test']}: {result['value']:.2f} (threshold: {result['threshold']:.2f})"
            )
        else:
