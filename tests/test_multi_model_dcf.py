...
class TestMultiModelDCF(unittest.TestCase):
    ...
    def test_deepseek_config_exists(self):
        ...
        self.assertTrue(
            deepseek_config_path.exists(), f"DeepSeek config not found at {deepseek_config_path}"
        )
    ...
    def test_config_isomorphism(self):
        ...
        for config_name, config in configs.items():
            ...
            self.assertEqual(
                len(missing_keys), 0, f"Config {config_name} missing keys: {missing_keys}"
            )
            self.assertEqual(
                len(extra_keys), 0, f"Config {config_name} has extra keys: {extra_keys}"
            )
        ...
        with patch.dict(
            ...
            try:
                ...
                if call_args and len(call_args) > 1:
                    kwargs = call_args[1]
                    self.assertTrue(
                        kwargs.get("fast_mode", False), "DeepSeek build should use fast_mode=True"
                    )
    ...
    def test_build_vs_fast_build_config_difference(self):
        ...
        other_configs = [
            f for f in self.config_dir.glob("*.yml") if not f.name.startswith("deepseek")
        ]
        ...
        self.assertNotEqual(
            deepseek_model, default_model, "Models should be different between configs"
        )
    ...
    def test_p3_script_fast_build_command(self):
        ...
        self.assertIn(
            "deepseek-r1:1.5b", script_content, "DeepSeek model not referenced in p3 script"
        )
