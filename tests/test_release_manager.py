...
class TestReleaseManager(unittest.TestCase):
    ...
    def test_collect_release_artifacts(self):
        ...
        expected_categories = ["reports", "manifests", "quality_reports", "llm_responses", "semantic_results", "configs"]
...
class TestReleaseManagerIntegration(unittest.TestCase):
    ...
    def setup_comprehensive_test_structure(self):
        ...
        for stage in ["extract", "transform", "load", "analysis"]:
            (quality_dir / f"stage_{stage}_report.json").write_text(f'{{"stage": "{stage}", "status": "completed"}}')
            (quality_dir / f"stage_{stage}_report.md").write_text(f"# {stage.title()} Report\\n\\nCompleted successfully")
        ...
        for ticker in tickers[:3]:  # Just first 3
            (semantic_dir / f"retrieved_docs_{ticker}_test.json").write_text(f'{{"ticker": "{ticker}", "docs": []}}')
    ...
    def test_end_to_end_release_workflow(self):
        ...
        expected_dirs = ["manifests", "llm_responses", "quality_reports", "configs", "semantic_results"]
