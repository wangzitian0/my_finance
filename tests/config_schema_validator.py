...
class ConfigSchemaValidator:
    ...
    def _validate_config_structure(self, filename: str, config: Dict[str, Any]) -> bool:
        ...
        if "tier" in config and config["tier"] not in self.valid_tiers:
            self.errors.append(
                f"{filename}: Invalid tier value {config['tier']}, must be one of {self.valid_tiers}"
            )
            success = False
    ...
    def _validate_company_data(self, filename: str, config: Dict[str, Any]) -> bool:
        ...
        if not has_companies and not has_reference:
            self.errors.append(
                f"{filename}: Must have either 'companies' section or 'reference_config'+'selected_companies'"
            )
            return False

        if has_companies and has_reference:
            self.warnings.append(
                f"{filename}: Has both 'companies' and 'reference_config' - using 'companies'"
            )
        ...
        if has_reference:
            if not isinstance(config["reference_config"], str) or not config[
                "reference_config"
            ].endswith(".yml"):
            ...
            if (
                not isinstance(config["selected_companies"], list)
                or not config["selected_companies"]
            ):
        ...
        for ticker, company_data in companies.items():
            ...
            if "cik" in company_data:
                ...
                if cik is not None:
                    ...
                    if not cik_str.isdigit() or len(cik_str) != 10:
                        self.errors.append(
                            f"{filename}: Company {ticker} has invalid CIK format: {cik}"
                        )
                        success = False
            ...
            if "market_cap_category" in company_data:
                ...
                if category not in self.valid_market_cap_categories:
                    self.errors.append(
                        f"{filename}: Company {ticker} has invalid market_cap_category: {category}"
                    )
                    success = False
    ...
    def _validate_data_sources(self, filename: str, config: Dict[str, Any]) -> bool:
        ...
        for source in required_sources:
            ...
            if "stage_config" not in source_config:
                self.errors.append(
                    f"{filename}: Data source '{source}' missing 'stage_config' field"
                )
                success = False
            elif not source_config["stage_config"].endswith(".yml"):
                self.errors.append(
                    f"{filename}: Data source '{source}' stage_config must be a YAML filename"
                )
                success = False
    ...
    def _validate_expected_files(self, filename: str, config: Dict[str, Any]) -> bool:
        ...
        for file_type in required_counts:
            if file_type not in expected_files:
                ...
                self.errors.append(
                    f"{filename}: Expected file count for '{file_type}' must be non-negative integer"
                )
                success = False
    ...
    def _validate_validation_config(self, filename: str, config: Dict[str, Any]) -> bool:
        ...
        if "required_success_rate" in validation:
            ...
            if not isinstance(rate, (int, float)) or not (0.0 <= rate <= 1.0):
                self.errors.append(
                    f"{filename}: 'required_success_rate' must be between 0.0 and 1.0"
                )
                success = False
