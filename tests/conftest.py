...
def sample_build_structure(temp_project_root: Path) -> Dict[str, Any]:
    ...
    (quality_dir / "quality_summary.json").write_text(
        '{"quality_score": 95, "errors": 0, "warnings": 2}'
    )
    (quality_dir / "quality_summary.md").write_text(
    ...
    (semantic_dir / "retrieved_docs_AAPL.json").write_text(
        '{"documents": ["doc1", "doc2"], "query": "AAPL financial data"}'
    )
