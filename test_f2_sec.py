...
def test_f2_sec():
    ...
    try:
        ...
        print(f"📊 Stock List: {config['stock_list']['name']} ({config['stock_list']['count']} companies)")
...
def test_f2_build():
    ...
    try:
        ...
        print(f"    - SEC Edgar enabled: {config.get('data_sources', {}).get('sec_edgar', {}).get('enabled', False)}")
