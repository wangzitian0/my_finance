...
def test_coordination_metrics():
    ...
    logger.info(f"  Parallel execution efficiency: {parallel_metrics['parallel_execution_efficiency']:.1%}")
...
def calculate_performance_improvements(baseline, parallel_plan, enhanced_plan):
    ...
    logger.info(f"  Agent utilization improvement: {projected_improvements['agent_utilization_improvement']}")
    logger.info(f"  Coordination overhead: {projected_improvements['coordination_overhead']}")
    logger.info(f"  Parallel batch efficiency: {projected_improvements['parallel_batch_efficiency']}")
