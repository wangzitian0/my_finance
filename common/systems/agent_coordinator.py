...
class AgentCoordinator:
    ...
    def execute_git_operation(self, command: str, description: str = "") -> TaskResult:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return TaskResult(success=True, output=result.stdout, execution_time=execution_time)
            else:
    ...
    def execute_p3_command(
        self, command: str, scope: str = "", description: str = ""
    ) -> TaskResult:
        ...
        try:
            ...
            return TaskResult(
                success=False, error="P3 command timed out", execution_time=time.time() - start_time
            )
        except Exception as e:
            return TaskResult(
                success=False, error=f"P3 command error: {str(e)}", execution_time=execution_time
            )
        finally:
            # Restore original working directory
            if "original_cwd" in locals() and original_cwd != Path.cwd():
    ...
    def execute_p3_create_pr(self, pr_title: str, issue_number: str) -> TaskResult:
        ...
        try:
            ...
            return TaskResult(
                success=False, error=f"P3 create-pr error: {str(e)}", execution_time=execution_time
            )
        finally:
            # Restore original working directory
            if "original_cwd" in locals() and original_cwd != Path.cwd():
    ...
    def execute_task_with_retry(self, task: WorkflowTask) -> TaskResult:
        ...
        for attempt in range(task.max_retries + 1):
            self.logger.info(
                f"Executing task (attempt {attempt + 1}/{task.max_retries + 1}): {task.description}"
            )
            ...
            if attempt < task.max_retries:
                ...
                self.logger.error(
                    f"Task failed after {task.max_retries + 1} attempts: {result.error}"
                )
                result.retry_count = attempt
                return result
    ...
    def _execute_direct_tools(self, task: WorkflowTask) -> TaskResult:
        ...
        if "git status" in task_lower:
            ...
            return self.execute_git_operation(
                'git commit -m "Auto commit by agent-coordinator"', "Commit changes"
            )
        ...
        # Default success for analysis tasks
            ...
            return TaskResult(
                success=True, output=f"Task completed: {task.description}", execution_time=1.0
            )
    ...
    def _execute_with_agent(self, task: WorkflowTask) -> TaskResult:
        ...
        if task.agent_type == AgentType.GIT_OPS:
            ...
            if "rebase" in task.description.lower():
                return self.execute_git_operation(
                    "git rebase main", "Git-ops agent: Rebase onto main"
                )
            ...
            if "env-status" in task.description.lower():
                return self.execute_p3_command(
                    "env-status", "", "Infra-ops agent: Check environment"
                )
            ...
            for i, task in enumerate(tasks):
                ...
                if not result.success:
                    ...
                    if task.priority > 7:  # Critical task failure
                        self.logger.error(
                            f"Critical task failed, stopping workflow: {task.description}"
                        )
                        break
    ...
    def _conclude_workflow(
        self, workflow_summary: Dict[str, Any], executed_tasks: List[WorkflowTask]
    ) -> Dict[str, Any]:
        """
        ...
        has_development_work = any(
            any(
                ...
                for keyword in ["implement", "fix", "build", "create", "update", "refactor"]
            )
        ...
        has_git_work = any(
            any(
                keyword in task.description.lower()
                for keyword in ["commit", "rebase", "merge", "git"]
            )
            for task in executed_tasks
        )
        ...
        has_analysis_work = any(
            any(
                keyword in task.description.lower()
                for keyword in ["analyze", "review", "validate", "check"]
            )
            for task in executed_tasks
        )
        ...
        if workflow_summary["workflow_success"]:
            if has_development_work or has_git_work:
                ...
                self.logger.info(
                    "Analysis work detected - recommending GitHub issue creation for follow-up"
                )
                conclusion_action = {
    ...
    def execute_full_automation_workflow(self) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Tasks: {workflow_result['tasks_successful']}/{workflow_result['tasks_executed']} successful"
        )
...
"""
        ...
        if result.success:
            ...
            self.logger.info(
                "PR includes: Claude Code hooks, logging integration, security enhancements"
            )
...
def delegate_task(description: str, agent_type: Optional[AgentType] = None) -> TaskResult:
    ...
    task = WorkflowTask(agent_type=agent_type, description=description, use_direct_tools=True)
