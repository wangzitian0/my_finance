...
try:
    ...
    print("Warning: Quality reporting not available")
...
class BuildTracker:
        ...
        **kwargs,
        ...
        if self.quality_reporter:
            try:
                ...
                logger.info(f"Quality report generated for {stage}")
    ...
    def complete_build(self, status: str = "completed") -> None:
        ...
        if self.quality_reporter:
            try:
                ...
                logger.info("Build quality summary report generated")
            except Exception as e:
    ...
    def generate_quality_report(self) -> bool:
        """Generate quality report for this build"""
        if not QUALITY_REPORTING_AVAILABLE:
            print("⚠️  Quality reporting not available")
            return False
            
        try:
            build_id = self.build_id
            json_path, md_path = generate_build_quality_report(build_id)
            
            # 添加到manifest
            if "quality_reports" not in self.manifest:
                self.manifest["quality_reports"] = {}
            
            self.manifest["quality_reports"] = {
                "json_report": str(json_path.relative_to(self.build_path.parent.parent)),
                "markdown_summary": str(md_path.relative_to(self.build_path.parent.parent)),
            ...
            print(f"✅ Quality report generated for build {build_id}")
            return True
