...
def find_python_packages(root_path: Path) -> List[Path]:
    ...
    skip_dirs = {'.git', '__pycache__', '.pytest_cache', '.pixi', 'build_data',
                '.venv', 'venv', 'node_modules', '.mypy_cache', '.coverage'}
    
    for path in root_path.rglob('*'):
        if (path.is_dir() and 
            path.name not in skip_dirs and 
            not any(part.startswith('.') for part in path.parts[len(root_path.parts):]) and
