...
class HRBPPRTracker:
    ...
    def record_pr_merge(self, pr_number: int) -> bool:
        ...
        if pr_record is None:
            self.logger.warning(f"Could not get complete info for PR #{pr_number}, using minimal record")
            pr_record = PRRecord(
                pr_number=pr_number,
        ...
        self.logger.info(f"PR #{pr_number} recorded. Total PRs: {counter_data['total_prs']}, Cycle PRs: {counter_data['cycle_prs']}")
    ...
    def _execute_hrbp_workflows_integrated(self, trigger: HRBPTrigger, workflows: List[str]) -> bool:
        """
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(f"Workflows executed: {', '.join(results.get('workflows_executed', []))}")
                
                # Log summary information
                summary = results.get('summary', {})
                if summary:
                    self.logger.info(f"Summary: {summary.get('workflows_completed', 0)} completed, "
                                   f"{summary.get('total_recommendations', 0)} recommendations generated")
            else:
                self.logger.error(f"Integrated HRBP workflow execution failed: {results.get('error', 'Unknown error')}")
    ...
    def _generate_performance_report(self, stats: Dict, trigger: HRBPTrigger) -> Dict:
        ...
        report = {
            ...
            'recommendations': self._generate_performance_recommendations(stats, performance_config)
        }
    ...
    def _generate_performance_recommendations(self, stats: Dict, config: Dict) -> List[str]:
        ...
        if success_rate < min_success_rate:
            recommendations.append(f"Overall success rate ({success_rate:.2%}) below threshold ({min_success_rate:.2%})")
        ...
        if avg_time > max_avg_time:
            recommendations.append(f"Average execution time ({avg_time}ms) exceeds threshold ({max_avg_time}ms)")
        ...
        for category, limit in error_limits.items():
            ...
            if count > limit:
                recommendations.append(f"Too many {category.replace('_max', '')} errors: {count} > {limit}")
        ...
        for agent, metrics in agent_performance.items():
            ...
            if agent_success_rate < min_success_rate:
                recommendations.append(f"Agent {agent} success rate ({agent_success_rate:.2%}) needs improvement")
    ...
    def _run_documentation_consolidation(self, trigger: HRBPTrigger) -> bool:
        ...
        try:
            ...
            report_file = self.logs_dir / f"hrbp_documentation_consolidation_{trigger.trigger_id}.json"
            with open(report_file, 'w') as f:
