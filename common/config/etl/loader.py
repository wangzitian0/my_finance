...
Moved from common/etl_loader.py → common/config/etl/loader.py (Issue #284)
"""
...
class ETLConfigLoader:
    ...
    def _load_yaml(self, file_path: Path) -> Dict[str, Any]:
        """Load YAML file with fallback defaults"""
        if not file_path.exists():
            # Return default config based on file type
            return self._get_default_config(file_path.name)

        try:
            with open(file_path, "r", encoding="utf-8") as f:
                return yaml.safe_load(f) or {}
        except yaml.YAMLError as e:
            # Return default on YAML error
            return self._get_default_config(file_path.name)
    ...
    def _get_default_config(self, filename: str) -> Dict[str, Any]:
        """Get default configuration based on filename"""
        if filename.startswith("stock_"):
            tier = filename.replace("stock_", "").replace(".yml", "")
            return {
                "description": f"Default {tier.upper()} stock configuration",
                "tier": tier,
                "max_size_mb": 100,
                "companies": {
        ...
        elif filename.startswith("source_"):
            source = filename.replace("source_", "").replace(".yml", "")
            return {
                "description": f"Default {source} data source configuration",
                "enabled": True,
                "api_config": {},
                "rate_limits": {"requests_per_second": 1},
                "data_types": ["quotes", "fundamentals"],
                "output_format": {"type": "json"},
            }
        elif filename.startswith("scenario_"):
            scenario = filename.replace("scenario_", "").replace(".yml", "")
            return {
                "description": f"Default {scenario} scenario configuration",
                "data_sources": ["yfinance", "sec_edgar"],
                "processing_mode": "test" if scenario == "dev" else "full",
                "output_formats": ["json"],
