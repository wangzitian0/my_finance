...
class TestBuildOutputTracking:
    ...
    def test_scan_and_track_outputs(self, tracker_with_outputs):
        ...
        with patch.object(tracker_with_outputs, '_save_manifest') as mock_save:
            ...
            assert any("AAPL_m7_daily_20250101.json" in path for path in real_outputs["yfinance_files"])
            ...
            assert tracker_with_outputs.manifest["statistics"]["files_processed"] == 3  # 2 yfinance + 1 sec
...
class TestBuildTrackerClassMethods:
    ...
    def test_get_latest_build_exists(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Create manifest
            manifest = {
                "build_info": {"build_id": "20250101_120000", "status": "completed"}
            }
            ...
            with patch('common.build.build_tracker.Path.cwd', return_value=project_root):
                with patch.object(Path, '__file__', project_root / "common" / "build" / "build_tracker.py"):
