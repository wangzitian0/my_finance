...
class TestEnvironmentCapture:
    ...
    def test_get_environment_state(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('common.monitoring.execution_monitor.Path.cwd', return_value=Path("/test/dir")):
            ...
            for days_ago in range(3):
                ...
                for i in range(5):  # 5 logs per day
                    log_entry = {
                        ...
                        "error_category": "medium" if results[i % len(results)] == ExecutionResult.FAILURE else None,
                        "execution_time_ms": (i + 1) * 100,
                        "retry_count": i % 3,
                        "environment_state": {},
                        "error_message": f"Error {i}" if results[i % len(results)] == ExecutionResult.FAILURE else None,
                        "stack_trace": None,
                        "command": None,
                        "working_directory": "/test"
                    }
    ...
    def test_monitor_execution_success(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch('common.monitoring.execution_monitor.get_monitor') as mock_get:
                ...
                mock_monitor.start_execution.assert_called_once_with("test-agent", "Test task", "test command")
    ...
    def test_monitor_execution_failure(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch('common.monitoring.execution_monitor.get_monitor') as mock_get:
                ...
                mock_monitor.start_execution.assert_called_once_with("test-agent", "Test task", None)
