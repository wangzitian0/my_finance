        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def secure_subprocess_run(self, args, timeout=30, **kwargs):
                ...
                try:
                    ...
                    raise TimeoutError(f"Command timed out after {timeout}s: {' '.join(validated_args)}")
...
class TestDirectoryOperationTimeouts:
    ...
    def timeout_directory_manager(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            dm.create_directories_with_timeout = create_directories_with_timeout.__get__(dm, DirectoryManager)
            dm.calculate_directory_size_with_timeout = calculate_directory_size_with_timeout.__get__(dm, DirectoryManager)
    ...
    def test_concurrent_access_error_recovery(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if total_operations > 0:
                ...
                assert success_rate > 0.8, f"Too many concurrent access failures: {success_rate:.2%} success rate"
