...
def mock_directory_config(temp_dir: Path) -> Dict[str, Any]:
    ...
    return {
        "storage": {
            "backend": "local_filesystem",
            "root_path": str(temp_dir / "build_data")
        },
        "paths": {
...
def mock_directory_manager(mock_project_root: Path, mock_directory_config: Dict[str, Any]):
    ...
    with patch('common.core.directory_manager.DirectoryManager') as mock_dm:
        ...
        mock_instance.get_layer_path.return_value = mock_project_root / "build_data" / "stage_00_raw"
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "integration: Integration tests for cross-module functionality")
