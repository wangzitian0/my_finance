...
class TestPathMigrationIntegration:
    ...
    def legacy_project_structure(self):
        ...
        legacy_structure = {
            "data": {
                ...
                "stage_01_extract": {
                    "extracted": ["processed_filings.json"]
                },
                "stage_02_transform": {
                ...
                "config": {
                    "list_magnificent_7.yml": {"companies": ["AAPL", "MSFT", "GOOGL"]}
                }
            },
        ...
        config_content = {
            ...
            "layers": {
                ...
                "layer_02_delta": {"subdirs": ["additions", "modifications", "deletions", "metadata"]},
                "layer_03_index": {"subdirs": ["vectors", "entities", "relationships", "embeddings", "indices"]},
                "layer_04_rag": {"subdirs": ["graph_db", "vector_store", "cache", "snapshots"]},
                "layer_05_results": {"subdirs": ["dcf_reports", "analytics", "exports", "dashboards", "api_responses"]}
            },
            "common": {
            ...
            if isinstance(content, dict):
                ...
                for file_name in content:
                    ...
                    if file_name.endswith('.json'):
                        ...
                        with open(file_path, 'w') as f:
                            yaml.dump(content[file_name] if isinstance(content[file_name], dict) else {"data": content[file_name]}, f)
                    else:
                        with open(file_path, 'w') as f:
    ...
    def test_complete_legacy_migration(self, legacy_project_structure):
        ...
        assert (build_data / "layer_05_results" / "dcf_reports" / "quarterly_analysis.json").exists()
    ...
    def test_mixed_legacy_new_environment(self, legacy_project_structure):
        ...
        assert (legacy_project_structure / "data" / "stage_00_original" / "new_data" / "legacy_file.txt").exists()
        assert (legacy_project_structure / "build_data" / "layer_01_raw" / "sec-edgar" / "new_file.txt").exists()
    ...
    def test_migration_validation_and_reporting(self, legacy_project_structure):
        ...
        for old_path, new_path in migrations:
            if old_path.exists():
                ...
                size = sum(f.stat().st_size for f in old_path.rglob("*") if f.is_file()) if old_path.is_dir() else old_path.stat().st_size
    ...
    def test_incremental_migration_support(self, legacy_project_structure):
        ...
        raw_migrations = [
            (old, new) for old, new in original_migrations
            if "stage_00_original" in str(old)
        ]
