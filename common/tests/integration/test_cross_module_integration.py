...
from common.core.directory_manager import DirectoryManager, DataLayer, StorageBackend
from common.core.config_manager import ConfigManager
from common.core.storage_manager import StorageManager, LocalFilesystemBackend
    ...
    def test_build_tracker_storage_manager_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            storage_manager = StorageManager(
                LocalFilesystemBackend(root_path=temp_dir)
            )
            ...
            artifact_path = tracker.save_artifact("stage_02_transform", "results.json", artifact_data)
    ...
    def test_agent_task_tracker_build_tracker_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            task_tracker.complete_task(
                ...
                environment_state={"build_id": build_id}
            )
    ...
    def test_build_tracker_schema_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            graph_results = {
                ...
                "node_types": {
                    "Stock": len(MAGNIFICENT_7_TICKERS),
                    "SECFiling": 21
                }
            }
            ...
            extraction_artifact = tracker.build_path / "artifacts" / "stage_01_extract_extraction_results.json"
    ...
    def test_end_to_end_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            config_data = {
                ...
                "logging": {
                    "level": "INFO",
                    "directory": str(Path(temp_dir) / "logs")
                }
            }
            ...
            with patch('common.core.directory_manager.DEFAULT_CONFIG_PATH', config_file):
                ...
                # Start build process
                build_id = build_tracker.start_build("end_to_end_integration", "pytest full integration")
                ...
                build_tracker.save_artifact("stage_01_extract", "extraction_summary.json", extraction_results)
                build_tracker.complete_stage("stage_01_extract", file_count=len(MAGNIFICENT_7_TICKERS))
                ...
                task_tracker.complete_task(
                    extract_task_id,
                    ExecutionResult.SUCCESS,
                    environment_state={"build_id": build_id, "stocks_processed": len(MAGNIFICENT_7_TICKERS)}
                )
                ...
                build_tracker.save_artifact("stage_02_transform", "transform_summary.json", transform_results)
                ...
                task_tracker.complete_task(
                    transform_task_id,
                    ExecutionResult.SUCCESS,
                    environment_state={"build_id": build_id, "nodes_created": len(MAGNIFICENT_7_TICKERS)}
                )
