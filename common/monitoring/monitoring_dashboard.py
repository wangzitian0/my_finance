...
class MonitoringDashboard:
    ...
    def _analyze_error_trends(self, error_patterns: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if agent_error_counts:
            ...
            insights.append(
                f"📈 {most_errors_agent[0]} has highest error count ({most_errors_agent[1]})"
            )
                ...
                critical_issues.append(
                    {
                        "type": (
                            "critical_error"
                            if error.get("error_category") == "critical"
                            else "high_frequency"
                        ),
                        ...
                        "urgency": (
                            "immediate" if error.get("error_category") == "critical" else "high"
                        ),
                    }
                )
    ...
    def print_summary_report(self, days: int = 7):
        ...
        print("-" * 20)
        print(
            f"Health Score:         {health['health_score']}/100 {status_emoji.get(health['status'], '❓')}"
        )
        ...
        if agent_perf:
            ...
            for agent, perf in sorted(
                agent_perf.items(), key=lambda x: x[1]["success_rate"], reverse=True
            ):
                success_rate = perf["success_rate"]
                total_tasks = perf["total_tasks"]
                status_emoji = (
                    "🟢" if success_rate >= 0.95 else "🟡" if success_rate >= 0.90 else "🔴"
                )
