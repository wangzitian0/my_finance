    ...
    def get_build_status(self) -> Dict[str, Any]:
        ...
        status.update({
            "dataset_summary": {
                ...
                "total_files": len(real_outputs.get("yfinance_files", [])) + len(real_outputs.get("sec_edgar_files", [])),
                "companies_processed": self.manifest["statistics"].get("companies_processed", 0),
            },
            "build_info": {
            ...
            "directory_structure": {
                ...
                "artifacts_count": len(list((self.build_path / "artifacts").glob("*"))) if (self.build_path / "artifacts").exists() else 0,
                "stage_logs_count": len(list((self.build_path / "stage_logs").glob("*"))) if (self.build_path / "stage_logs").exists() else 0,
            }
        })
