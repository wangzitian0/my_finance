...
class ToolManager:
    ...
    def create_tool_workspace(self, tool_name: str, timestamp: Optional[str] = None) -> Optional[ToolExecutionContext]:
        """
        ...
        try:
            ...
            for layer_name in config.input_layers:
                try:
                    ...
                    self.logger.warning(f"Invalid input layer '{layer_name}' for tool '{tool_name}'")
            ...
            for layer_name in config.output_layers:
                try:
                    ...
                    self.logger.warning(f"Invalid output layer '{layer_name}' for tool '{tool_name}'")
    ...
    def cleanup_tool_workspace(self, context: ToolExecutionContext, remove_workspace: bool = False):
        ...
        try:
            ...
            if workspace_key in self._workspace_metadata:
                self._workspace_metadata[workspace_key]['cleaned_up_at'] = datetime.now().isoformat()
    ...
    def execute_tool(self, tool_name: str, timestamp: Optional[str] = None, 
                    cleanup_on_completion: bool = False) -> bool:
        """
            ...
            if timestamp_dir.is_dir() and len(timestamp_dir.name) == 15:  # YYYYMMDD_HHMMSS
                try:
                    ...
                    if dir_timestamp < cutoff_date:
                        ...
                        keys_to_remove = [k for k in self._active_workspaces.keys() 
...
def create_tool_workspace(tool_name: str, timestamp: Optional[str] = None) -> Optional[ToolExecutionContext]:
