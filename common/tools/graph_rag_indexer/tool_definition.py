...
class GraphRAGIndexer(BaseTool):
    ...
    def _extract_entities_relationships(self, context: ToolExecutionContext) -> bool:
        ...
        try:
            ...
            extraction_summary = {
                ...
                "entities_by_type": {
                    etype: len(entities) for etype, entities in extracted_entities.items()
                },
    ...
    def _build_knowledge_graph(self, context: ToolExecutionContext) -> bool:
        ...
        try:
            ...
            graph_stats = {
                ...
                "relationship_types": {},
                "graph_density": (
                    len(relationships) / (len(all_entities) * (len(all_entities) - 1))
            ...
            for entity in all_entities.values():
                ...
                graph_stats["node_types"][entity_type] = (
                    graph_stats["node_types"].get(entity_type, 0) + 1
                )
            ...
            context.add_message(
                f"Knowledge graph built: {len(all_entities)} nodes, {len(relationships)} relationships"
            )
            return True
    ...
    def _create_vector_indices(self, context: ToolExecutionContext) -> bool:
        ...
        try:
            ...
            with open(index_file, "wb") as f:
                f.write(
                    b"FAISS_INDEX_PLACEHOLDER"
                )  # In real implementation, save actual FAISS index
    ...
    def _get_vector_store_info(self, context: ToolExecutionContext) -> Dict:
        ...
        metadata_file = (
            context.workspace_path / "vector_store" / "metadata" / "vector_index_metadata.json"
        )
        try:
    ...
    def validate_outputs(self, context: ToolExecutionContext) -> bool:
        ...
        try:
            ...
            if graph_stats["total_nodes"] < min_nodes:
                context.add_message(
                    f"Insufficient nodes in graph: {graph_stats['total_nodes']} < {min_nodes}",
                    "ERROR",
                )
                return False
            ...
            if graph_stats["total_relationships"] < min_relationships:
                context.add_message(
                    f"Insufficient relationships in graph: {graph_stats['total_relationships']} < {min_relationships}",
                    "ERROR",
                )
                return False
            ...
            for layer_name in self.config.output_layers:
                if layer_name in context.output_paths:
                    ...
                    update_files = list(
                        layer_path.glob(f"graph_rag_update_{context.timestamp}.json")
                    )
                    if not update_files:
                        context.add_message(
            ...
            context.add_message(
                f"Graph: {graph_stats['total_nodes']} nodes, {graph_stats['total_relationships']} relationships"
            )
