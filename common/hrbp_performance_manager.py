...
class HRBPPerformanceManager:
    ...
    def _get_default_config(self) -> Dict:
        ...
        return {
            "agent_performance": {
                ...
                "error_categories": {"critical_max": 0, "high_max": 2, "medium_max": 5},
            },
            "coordination": {
    ...
    def _assess_capability_level(
        self, success_rate: float, avg_time_ms: float
        ...
        for primary, secondaries in common_interactions:
            if primary in self.agent_registry:
                ...
                metrics = CoordinationMetrics(
                    ...
                    coordination_time_ms=int(
                        45000 * (1.1 - effectiveness)
                    ),  # Better coordination = faster
            ...
            if metrics.success_rate < self.performance_thresholds.get("success_rate_minimum", 0.85):
                ...
                recommendations.append(
                    PerformanceOptimizationRecommendation(
                        ...
                        target_metrics={
                            "success_rate": self.performance_thresholds["success_rate_minimum"]
                        },
        ...
        for coord_metrics in ineffective_coordinations:
            ...
            recommendations.append(
                PerformanceOptimizationRecommendation(
                    ...
                    priority=(
                        "high" if coord_metrics.effectiveness_level == "ineffective" else "medium"
                    ),
                    ...
                    target_metrics={
                        "coordination_time_ms": coord_metrics.coordination_time_ms * 0.8
                    },
                    implementation_steps=[
    ...
    def _save_optimization_recommendations(
        self, recommendations: List[PerformanceOptimizationRecommendation]
    ):
        ...
        try:
            ...
            history.append(
                {"timestamp": datetime.now().isoformat(), "recommendations": serializable_data}
            )

            # Keep only recent history (90 days by default)
            retention_days = self.config.get("optimization", {}).get(
                "recommendation_retention_days", 90
            )
            ...
            self.logger.info(
                f"Saved {len(recommendations)} optimization recommendations to history"
            )
    ...
    def run_comprehensive_performance_analysis(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            report = {
                ...
                "agent_performance": {
                    name: asdict(metrics) for name, metrics in performance_data.items()
                },
            ...
            report_file = (
                self.logs_dir
                / f"hrbp_comprehensive_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
            with open(report_file, "w") as f:
                json.dump(report, f, indent=2)

            self.logger.info(
                f"Comprehensive performance analysis completed in {analysis_time:.2f}s"
            )
    ...
    def _calculate_system_metrics(
        ...
        if coordination_data:
            coordination_effectiveness = [
                1.0 if coord.coordination_success else 0.0 for coord in coordination_data
            ]

        avg_coordination_effectiveness = (
            sum(coordination_effectiveness) / len(coordination_effectiveness)
            if coordination_effectiveness
            else 0
        )
        ...
        return {
            ...
            "total_agents_active": len(
                [m for m in performance_data.values() if m.total_executions > 0]
            ),
    ...
    def _generate_executive_summary(
        ...
        critical_agents = len(
            [m for m in performance_data.values() if m.capability_level == "critical"]
        )

        # Performance highlights
        best_performers = sorted(
            performance_data.values(), key=lambda x: x.success_rate, reverse=True
        )[:3]
        ...
        return {
            ...
            "top_recommendations": [
                {"agent": rec.agent_name, "priority": rec.priority, "description": rec.description}
                for rec in sorted(
                    recommendations,
    ...
    def _determine_overall_health(self, system_metrics: Dict[str, Any]) -> str:
        ...
        if (
            success_rate >= 0.95
            and coordination_effectiveness >= 0.90
            and agents_needing_attention == 0
        ):
            return "excellent"
        elif (
            success_rate >= 0.85
            and coordination_effectiveness >= 0.80
            and agents_needing_attention <= 2
        ):
            return "good"
        elif (
            success_rate >= 0.70
            and coordination_effectiveness >= 0.60
            and agents_needing_attention <= 5
        ):
