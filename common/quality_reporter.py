...
class QualityReporter:
    ...
    def report_stage_quality(self, stage: str, partition: Optional[str] = None, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Quality report completed for {stage}: {quality_data.get('overall_success_rate', 'N/A')} success rate")
            return quality_data
    ...
    def _analyze_sec_extraction(self, partition: str) -> Dict[str, Any]:
        ...
        try:
            ...
            success_rate = total_documents / max(expected_total_docs, 1) if expected_total_docs > 0 else 0
    ...
    def _analyze_transform_stage(self, partition: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if partition:
                ...
                partition_path = max(partition_dirs, key=lambda x: x.name) if partition_dirs else None
            ...
            cleaned_files = len(list(partition_path.glob("**/cleaned/*.json"))) if (partition_path / "cleaned").exists() else 0
            enriched_files = len(list(partition_path.glob("**/enriched/*.json"))) if (partition_path / "enriched").exists() else 0
            normalized_files = len(list(partition_path.glob("**/normalized/*.json"))) if (partition_path / "normalized").exists() else 0
    ...
    def _analyze_load_stage(self, partition: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            graph_nodes = len(list(partition_path.glob("**/graph_nodes/*.json"))) if (partition_path / "graph_nodes").exists() else 0
            embeddings = len(list(partition_path.glob("**/embeddings/*.npy"))) if (partition_path / "embeddings").exists() else 0
            dcf_results = len(list(partition_path.glob("**/dcf_results/*.json"))) if (partition_path / "dcf_results").exists() else 0
    ...
    def _analyze_analysis_stage(self, partition: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            success_rate = companies_analyzed / max(expected_companies, 1) if expected_companies > 0 else 0
    ...
    def _analyze_reporting_stage(self, partition: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            success_rate = reports_generated / max(expected_reports, 1) if expected_reports > 0 else 0
...
"""
    ...
    def generate_build_summary_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            overall_health = sum(stage_success_rates) / len(stage_success_rates) if stage_success_rates else 0
            ...
            summary = {
                ...
                "stages_successful": sum(1 for data in self.stage_reports.values() 
