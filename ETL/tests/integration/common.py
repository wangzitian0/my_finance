...
def ensure_common_tables():
    ...
    1) fetch_log: record data fetching status
...
def can_fetch(ticker: str, dimension: str, cooldown_minutes=60) -> bool:
    ...
    2. If last fetch succeeded, but data is stale (beyond cooldown time), allow fetching
    ...
    # If last time failed, must retry
    if success == 0:
        return True
...
def update_fetch_time(ticker: str, dimension: str, success: bool):
    """
    Update in fetch_log:
    - `last_fetch_time` (update regardless of success or failure)
