...
The following are CIK numbers for the Magnificent 7 (7 major tech companies):
...
It's recommended to use the above CIK numbers directly in the configuration file instead of stock ticker symbols,
so the program will directly use CIK to query filings data, avoiding errors caused by internal conversion when requesting /files/company_tickers.json.
"""
...
# Set logging output level to DEBUG
...
def is_file_recent(filepath, hours=1):
    """
    Check if file was modified within the specified hours in the past
    Parameters:
      filepath: file path
...
def run_job(config_path):
    """
    Main task: Load configuration from YAML configuration file, and process each CIK and filing type sequentially,
    directly call secedgar.filings object's save() method to save filings data.
    Data will be saved under: data/stage_01_extract/sec_edgar/<date_partition>/<ticker>/ directory.
    Parameters:
    ...
    logging.info(f"Starting task processing, total {total_tasks} tasks")
    ...
    for cik in cik_list:
        logging.info(f"Directly using CIK for query: {cik}")
        ...
        for ft in file_types:
            logging.info(f"Starting to process {ft} filings for {cik}")
            ...
            try:
                ...
                # Directly set cik_lookup internal mapping to avoid calling /files/company_tickers.json
                ...
                logging.info(
                    "Please check access permissions for /files/company_tickers.json, if issues persist please manually download that file."
                )
            pbar.update(1)
            time.sleep(3)
