...
from common.graph_rag_schema import (
    ...
    NewsEventNode, FinancialMetricsNode, GraphRelationship,
    RelationshipType, DocumentType, MAGNIFICENT_7_CIKS,
    ETLStageOutput
)
...
class GraphDataIntegrator:
    ...
    def integrate_m7_data(self, data_dir: Path) -> ETLStageOutput.GraphNodesOutput:
        ...
        try:
            ...
            logger.info(f"Graph data integration completed. Total nodes: {stats['nodes_created']}, "
    ...
    def _create_stock_node(self, ticker: str) -> Dict[str, int]:
        ...
        info = company_info.get(ticker, {"name": f"{ticker} Inc.", "sector": "Unknown", "industry": "Unknown"})
        ...
        for sec_file in sec_files:
            try:
                ...
                if len(filename_parts) >= 5:
                    ...
                    accession_number = filename_parts[5] if len(filename_parts) > 5 else filename_parts[4]
    ...
    def _integrate_yfinance_data(self, ticker: str, data_dir: Path) -> Dict[str, int]:
        ...
        for partition_dir in sorted(yf_dir.iterdir(), reverse=True):
            ...
            try:
                ...
                stats['nodes_created'] += 1
                stats['relationships_created'] += 1
                stats['node_types']['FinancialMetrics'] = stats['node_types'].get('FinancialMetrics', 0) + 1
        ...
        for dcf_file in dcf_files:
            try:
                ...
                if ticker in dcf_data:
                    ...
                    stats['nodes_created'] += 1
                    stats['relationships_created'] += 1
                    stats['node_types']['DCFValuation'] = stats['node_types'].get('DCFValuation', 0) + 1
