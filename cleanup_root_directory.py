...
def analyze_root_directory() -> Tuple[List[Path], List[Path], List[Path], List[Path]]:
    ...
    for item in root_path.iterdir():
        if item.is_file():
            ...
            elif item.suffix in ['.yml', '.yaml', '.toml', '.ini', '.cfg'] and 'config' in item.name.lower():
...
def validate_clean_structure() -> Tuple[int, List[str]]:
    ...
    core_directories = {
        ...
        'evaluation', 'templates', 'build_data', 'tests', '.git'
    }
...
def execute_directory_cleanup():
    ...
    success = (
        removed_coverage >= 15 and  # Expect at least 15 coverage files removed
        moved_tests >= 3 and        # Expect at least 3 test files moved
        reduction_percent >= 70     # Target 70% reduction achieved
    )
