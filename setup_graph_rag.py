...
class GraphRAGSetup:
    ...
    def check_python_version(self):
        ...
        if current_version < self.min_python_version:
            logger.error(f"Python {'.'.join(map(str, self.min_python_version))} or higher is required")
            logger.error(f"Current version: {'.'.join(map(str, current_version))}")
            sys.exit(1)
        
        if current_version < self.recommended_python_version:
            logger.warning(f"Python {'.'.join(map(str, self.recommended_python_version))} or higher is recommended")
    ...
    def install_package(self, package: str):
        ...
        try:
            ...
            if 'torch' in package_name:
                logger.info("For PyTorch, you might want to visit https://pytorch.org/get-started/locally/")
...
def main():
    ...
    try:
        ...
        print("3. Set up Neo4j database using: pixi run setup-env")
