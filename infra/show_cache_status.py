...
def check_brew_cache() -> Dict:
    ...
    cache_info = {"podman_cached": "❓ Unknown", "cache_size": "❓ Unknown", "details": []}
...
def check_custom_caches() -> Dict:
    ...
    cache_info = {"setup_marker": "❓ Unknown", "env_marker": "❓ Unknown", "details": []}
...
def main():
    ...
    # Detailed info
    print("\n📋 Details:")
    all_details = podman_cache["details"] + brew_cache["details"] + custom_cache["details"]
    ...
    print(f"\n💡 Cache Summary:")
    print(
        f"   - Podman itself: {'✅ Cached via Homebrew' if brew_cache['podman_cached'].startswith('✅') else '❌ Not cached'}"
    )
    print(
        f"   - Container images: {'✅ ' + podman_cache['storage_size'] if podman_cache['storage_size'] != '❓ Unknown' else '❌ No storage'}"
    )
    print(
        f"   - Machine setup: {'✅ Ready' if custom_cache['env_marker'].startswith('✅') else '❌ Needs setup'}"
    )
