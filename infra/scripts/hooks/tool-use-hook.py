...
def parse_arguments():
    ...
    parser.add_argument(
        "--tool-name", type=str, required=True, help="Name of the tool being invoked"
    )

    parser.add_argument("--parameters", type=str, help="Tool parameters as JSON string")

    parser.add_argument("--response", type=str, help="Tool response as JSON string")
    ...
    parser.add_argument(
        "--event-id", type=str, help="Event ID from start phase (for completion tracking)"
    )

    parser.add_argument("--execution-time", type=int, help="Execution time in milliseconds")
    ...
    parser.add_argument("--error-message", type=str, help="Error message if tool execution failed")

    parser.add_argument("--session-id", type=str, help="Override session ID")

    parser.add_argument(
        "--max-response-length", type=int, default=10000, help="Maximum response length to capture"
    )
