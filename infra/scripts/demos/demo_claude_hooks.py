...
def simulate_claude_interaction():
    ...
    session_id = hook_manager.start_session(
        {"user": "demo_user", "interface": "claude_code", "version": "1.0.0"}
    )
    ...
    prompt_event = hook_manager.capture_user_prompt(
        user_prompt.strip(), {"timestamp": time.time(), "interface": "web", "user_id": "demo_user"}
    )
    ...
    **Investment Recommendation**: BUY
    ...
    response_event = hook_manager.capture_ai_response(
        final_response.strip(), thinking.strip(), generation_time
    )
    ...
    try:
        ...
        error_event = hook_manager.capture_error(
            ...
            context={"operation": "sec_edgar_fetch", "symbol": "AAPL", "retry_count": 0},
