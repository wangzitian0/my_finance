        ...
        for root, dirs, files in os.walk(self.root_dir):
            ...
            for file in files:
                for pattern in artifact_patterns:
                    if (
                        fnmatch.fnmatch(file, pattern) and file != "pixi.lock"
                    ):  # pixi.lock is allowed
                ...
                self.recommendations.append(
                    {
                        ...
                        "script": self._generate_move_script(
                            violations, "build_data/stage_00_raw/"
                        ),
                    }
                )
    ...
    def _generate_report(self) -> Dict:
        ...
        report = {
            ...
            "data_architecture_status": {
                ...
                "existing_layers": len(
                    [p for p in self.required_data_layers.keys() if (self.root_dir / p).exists()]
                ),
            },
        }
...
def main():
    ...
    print(
        f"Data Architecture: {report['data_architecture_status']['existing_layers']}/{report['data_architecture_status']['required_layers']} layers"
    )
