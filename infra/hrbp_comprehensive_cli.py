...
Examples:
  ...
  python infra/hrbp_comprehensive_cli.py data agents --format json
    ...
    parser.add_argument(
        'command',
        choices=[
            'status', 'integration-health', 'performance', 'coordination', 
    ...
    parser.add_argument(
        'subcommand',
        nargs='?',
        help='Subcommand for the main command'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help='Number of days for historical analysis (default: 30)'
    )
    ...
    if summary:
        ...
        if top_recs:
            ...
            for i, rec in enumerate(top_recs[:5], 1):
                ...
                print(f"   {i}. {priority_icon} {rec.get('agent', 'unknown')}: {rec.get('description', 'No description')}")
...
def execute_coordination_command(args):
    ...
    effectiveness_icon = '🟢' if avg_effectiveness > 0.8 else '🟡' if avg_effectiveness > 0.6 else '🔴'
    ...
    if coordination_metrics:
        ...
        effective_count = len([m for m in coordination_metrics if m.effectiveness_level in ['highly_effective', 'effective']])
    ...
    efficiency_icon = '🟢' if resource_efficiency > 0.8 else '🟡' if resource_efficiency > 0.6 else '🔴'
...
def execute_workflow_history(args):
    ...
    for i, trigger in enumerate(reversed(history), 1):
        ...
        print(f"   PR Range: {trigger.get('starting_pr', '')}-{trigger.get('ending_pr', '')} ({trigger.get('pr_count', 0)} PRs)")
...
def execute_metrics_data(args):
    ...
    execution_times = [a.average_execution_time_ms for a in performance_data.values() if a.average_execution_time_ms > 0]
...
def execute_export_data(args):
    ...
    export_data = {
        ...
        'agent_performance': {
            name: asdict(metrics) 
            for name, metrics in performance_manager.collect_agent_performance_data(args.days).items()
        },
