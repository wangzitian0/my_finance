...
Examples:
  ...
  python infra/hrbp_automation.py history --limit 5
    ...
    parser.add_argument(
        ...
        nargs='?',
        type=int,
        help='PR number (required for record-pr command)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help='Limit for history command (default: 10)'
    )
    ...
    try:
        ...
        if args.command == 'status':
            ...
            confirmation = input("Are you sure you want to manually trigger HRBP cycle? [y/N]: ").strip().lower()
...
def print_history(history):
    ...
    for i, trigger in enumerate(reversed(history), 1):
        ...
        print(f"   Status: {trigger['status']}")
        print(f"   PR Range: {trigger['starting_pr']}-{trigger['ending_pr']} ({trigger['pr_count']} PRs)")
        print(f"   Timestamp: {trigger['timestamp']}")
        print(f"   Workflows: {', '.join(trigger['workflows_triggered'])}")
