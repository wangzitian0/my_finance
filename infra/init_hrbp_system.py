...
Examples:
  ...
  python infra/init_hrbp_system.py --reset            # Reset and reinitialize
    ...
    parser.add_argument(
        '--validate-only',
        action='store_true',
        help='Only run validation checks, do not initialize'
    )
    
    parser.add_argument(
        '--reset',
        action='store_true',
        help='Reset existing HRBP data before initialization'
    )
    ...
    try:
        ...
        if success:
            ...
            if not args.validate_only:
                ...
                print("   2. Run performance analysis: python infra/hrbp_comprehensive_cli.py performance")
                print("   3. Check integration health: python infra/hrbp_comprehensive_cli.py integration-health")
            sys.exit(0)
        else:
...
def setup_integrations(verbose: bool = False) -> Dict:
    ...
    try:
        ...
        if verbose:
            ...
            print(f"   {status_icon} P3 system integration: {'successful' if p3_success else 'failed'}")
        
        # Test Git-ops integration
        git_success = framework.integrate_with_git_ops()
        if verbose:
            status_icon = "✅" if git_success else "❌" 
            print(f"   {status_icon} Git-ops integration: {'successful' if git_success else 'failed'}")
...
def validate_system_health(verbose: bool = False) -> Dict:
    ...
    try:
        ...
        if overall_status in ['healthy', 'degraded']:
            ...
            return {
                'success': False, 
                'error': f'System health: {overall_status}',
                'issues': issues
            }
...
def show_system_info():
    ...
    try:
        ...
        print(f"   Git-ops Integration: {'✅' if integration_status.get('git_ops_integration') else '❌'}")
