...
class MultiStepReasoning:
    ...
    def decompose_question(self, complex_question: str) -> List[str]:
        ...
        if any(pattern in question_lower for pattern in ['recent news', 'latest development', 'recent event']):
    ...
    def answer_sub_question(self, 
                           sub_question: str, 
                           accumulated_context: Dict[str, Any],
                           step_number: int) -> ReasoningStep:
        """
    ...
    def _execute_graph_query(self, query_info: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def synthesize_final_answer(self, 
        ...
        for i, step in enumerate(reasoning_steps, 1):
            ...
            if step.confidence < 0.5:
                answer_parts.append(f"*Note: Lower confidence ({step.confidence:.2f}) - limited data available*")
    ...
    def _assess_complexity(self, original_question: str, sub_questions: List[str]) -> float:
        ...
        complexity_indicators = [
            ...
            sum(1 for pattern in self.complex_question_patterns 
    ...
    def _extract_evidence(self, 
                         ...
                         semantic_content: List) -> List[str]:
    ...
    def is_complex_question(self, question: str) -> bool:
        ...
        complexity_indicators = [
            ...
            # Temporal complexity
            any(temporal in question_lower for temporal in ['recent', 'latest', 'trend', 'historical', 'over time']),
            
            # Causal relationships
            any(causal in question_lower for causal in ['impact', 'affect', 'influence', 'because of', 'due to']),
            
            # Multiple question types
            '?' in question and question.count('?') > 1,
            
            # Explicit complexity markers
            any(marker in question_lower for marker in ['comprehensive', 'detailed', 'thorough', 'complete analysis']),
            
            # Pattern matching
            any(pattern in question_lower for pattern in [
                'based on', 'considering', 'taking into account', 'given that',
                'compare and', 'analyze the relationship', 'how does this affect'
            ])
        ]
