...
This system implements a Graph Retrieval-Augmented Generation (Graph RAG) system specifically designed for financial analysis and investment decision support for the Magnificent 7 (M7) companies.

## Core Features

### 1. Semantic Embedding
- Generate document semantic vectors using Sentence Transformers
- Support chunking and embedding of SEC filings and news content
...
- Vector similarity-based content retrieval
- Intelligent ranking combining recency and relevance
...
- Support multiple scenarios including DCF valuation, risk analysis, and comparative analysis
...
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│  User Question  │ -> │ Query Generator │ -> │  Neo4j Graph DB │
└─────────────────┘    └─────────────────┘    └─────────────────┘
         │                                              │
         v                                              v
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│Semantic Retrieval│ <- │Reasoning Engine │ <- │ Structured Data │
└─────────────────┘    └─────────────────┘    └─────────────────┘
         │                       │                       
         v                       v                       
┌─────────────────┐    ┌─────────────────┐              
...
4. **News Impact Analysis**: "How do recent news events affect Netflix's stock price?"
...
semantic_content = retriever.retrieve_relevant_content(
    ...
    graph_data=data,
    top_k=10,           # Return top 10 relevant contents
...
1. Add new intent to `QueryIntent` enumeration
...
1. **NumPy compatibility warnings**: Normal occurrence, does not affect functionality
...
- Create a [GitHub Issue](https://github.com/wangzitian0/my_finance/issues)
