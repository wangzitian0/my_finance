...
from ETL.models import (
    Stock, Info, FastInfo, PriceData, Recommendations, Sustainability,
    SECFiling, NewsEvent, DCFValuation, FinancialMetrics, DocumentChunk
)
...
class GraphRAGDataIngestion:
    ...
    def ingest_m7_data(self) -> Dict[str, Any]:
        ...
        logger.info(f"M7 data ingestion completed. Processed {ingestion_stats['companies_processed']} companies")
    ...
    def _create_or_update_stock(self, ticker: str, yfinance_data: Dict[str, Any]) -> Optional[Stock]:
        """Create or update Stock node."""
        try:
        ...
        try:
            ...
            for date_str, price_info in history_data.items():
                try:
                    ...
                    logger.warning(f"Error processing price data for {stock.ticker} on {date_str}: {e}")
                    continue
    ...
    def _create_sec_filing_node(self, 
                               ticker: str, 
                               cik: str, 
                               filing_type: str, 
                               filing_data: Dict[str, Any],
                               filing_file: Path) -> Optional[SECFiling]:
        ...
        try:
            ...
            sec_filing = SECFiling(
                ...
                parsing_success=bool(filing_data.get('business_overview') or filing_data.get('raw_content'))
            )
    ...
    def _generate_filing_embeddings(self, sec_filing: SECFiling, filing_data: Dict[str, Any]) -> int:
        ...
        try:
            ...
            logger.debug(f"Generated {embeddings_generated} embeddings for SEC filing {sec_filing.accession_number}")
    ...
    def create_sample_dcf_valuations(self) -> Dict[str, Any]:
        ...
        for ticker, dcf_data in sample_dcf_data.items():
            try:
                stock = Stock.nodes.get_or_none(ticker=ticker)
                if stock:
                    upside_downside = (dcf_data['intrinsic_value'] - dcf_data['current_price']) / dcf_data['current_price']
