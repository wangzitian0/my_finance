...
class IntelligentAnswerGenerator:
    ...
    def generate_answer(self, 
                       ...
                       semantic_content: List[RetrievalResult]) -> Dict[str, Any]:
        """
    ...
    def _process_graph_data(self, graph_data: Dict[str, Any], context: Dict[str, Any]):
        ...
        if dcf:
            ...
            context['sources'].append(f"DCF Valuation ({dcf.get('valuation_date', 'Unknown date')})")
    ...
    def _process_semantic_content(self, semantic_content: List[RetrievalResult], context: Dict[str, Any]):
        ...
        for result in semantic_content:
            context['document_content'].append({
                ...
                'document_date': result.document_date.isoformat() if isinstance(result.document_date, datetime) else str(result.document_date)
            })
    ...
    def _generate_context_summary(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        quality_indicators = ['dcf_valuation', 'financial_metrics', 'company_info', 'sec_filings']
        available_indicators = sum(1 for indicator in quality_indicators if indicator in structured_data)
        ...
        if dates:
            try:
                latest_date = max(datetime.fromisoformat(date.replace('Z', '+00:00')) if isinstance(date, str) else date for date in dates)
    ...
    def generate_dcf_analysis_answer(self, question: str, context: Dict[str, Any]) -> str:
        ...
        risk_content = [c for c in context['document_content'] 
        ...
        if risk_content:
            ...
            for risk in risk_content[:2]:  # Top 2 risk-related content pieces
                ...
                content = risk['content'][:100] + "..." if len(risk['content']) > 100 else risk['content']
    ...
    def generate_comparison_answer(self, question: str, context: Dict[str, Any]) -> str:
        ...
        comparison_content = [c for c in context['document_content'] 
                            if any(word in c['content'].lower() for word in ['comparison', 'versus', 'competitive', 'peer'])]
        
        if comparison_content:
            answer_parts.append(f"\n**Market Context:**")
            for content in comparison_content[:2]:
                summary = content['content'][:150] + "..." if len(content['content']) > 150 else content['content']
    ...
    def generate_risk_analysis_answer(self, question: str, context: Dict[str, Any]) -> str:
        ...
        if metrics:
            ...
            if debt_to_equity or current_ratio:
                ...
                if debt_to_equity:
                    risk_level = "High" if debt_to_equity > 2 else "Moderate" if debt_to_equity > 1 else "Low"
                    answer_parts.append(f"- Debt-to-Equity: {debt_to_equity:.2f} ({risk_level} leverage)")
                if current_ratio:
                    liquidity = "Good" if current_ratio > 1.5 else "Adequate" if current_ratio > 1 else "Poor"
                    answer_parts.append(f"- Current Ratio: {current_ratio:.2f} ({liquidity} liquidity)")
    ...
    def generate_news_impact_answer(self, question: str, context: Dict[str, Any]) -> str:
        ...
        if news_events:
            ...
            sorted_news = sorted(news_events, key=lambda x: x.get('published_date', ''), reverse=True)
            ...
            for news in sorted_news[:5]:  # Top 5 most recent
                ...
                if impact_categories:
                    impacts = [category for category, relevance in impact_categories.items() if relevance > 0.5]
                    if impacts:
                        answer_parts.append(f"  Impact areas: {', '.join(impacts)}")
        
        # Add relevant news content from semantic retrieval
        news_content = [c for c in context['document_content'] 
                       if c.get('document_type') == 'news']
        if news_content:
            answer_parts.append(f"\n**Key News Highlights:**")
            for content in news_content[:3]:
                summary = content['content'][:120] + "..." if len(content['content']) > 120 else content['content']
                answer_parts.append(f"- {summary}")
    ...
    def generate_sector_analysis_answer(self, question: str, context: Dict[str, Any]) -> str:
        ...
        sector_content = [c for c in context['document_content'] 
                         if any(word in c['content'].lower() for word in ['sector', 'industry', 'competitive', 'market share'])]
    ...
    def generate_historical_analysis_answer(self, question: str, context: Dict[str, Any]) -> str:
        ...
        if len(metrics) > 1:
            ...
            answer_parts.append(f"**Financial Performance Trends ({len(sorted_metrics)} periods):**")
            ...
            if len(sorted_metrics) >= 2:
                ...
                revenue_trend = self._calculate_trend(previous.get('revenue'), recent.get('revenue'))
                if revenue_trend:
                    answer_parts.append(f"- Revenue trend: {revenue_trend}")
                
                fcf_trend = self._calculate_trend(previous.get('free_cash_flow'), recent.get('free_cash_flow'))
                if fcf_trend:
                    answer_parts.append(f"- Free Cash Flow trend: {fcf_trend}")
        
        # Add historical context from semantic content
        historical_content = [c for c in context['document_content'] 
                            if any(word in c['content'].lower() for word in ['historical', 'trend', 'growth', 'performance'])]
    ...
    def generate_general_answer(self, question: str, context: Dict[str, Any]) -> str:
        ...
        if context['document_content']:
            top_content = sorted(context['document_content'], 
    ...
    def _calculate_trend(self, previous_value: Optional[float], current_value: Optional[float]) -> Optional[str]:
