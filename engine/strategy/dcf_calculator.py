...
class DCFCalculator:
    ...
    def calculate_dcf(self, inputs: DCFInputs, graph_rag_context: Optional[Dict] = None) -> DCFResults:
        """
    ...
    def _project_cash_flows(self, inputs: DCFInputs) -> List[Decimal]:
        ...
        for i in range(inputs.projection_years):
            growth_rate = inputs.growth_rates[i] if i < len(inputs.growth_rates) else inputs.growth_rates[-1]
    ...
    def _calculate_terminal_value(self, inputs: DCFInputs, final_year_cf: Decimal) -> Decimal:
        ...
        if inputs.terminal_multiple:
            ...
            terminal_value = terminal_cf / (self._calculate_wacc(inputs) - inputs.terminal_growth_rate)
    ...
    def _calculate_present_values(self, cash_flows: List[Decimal], discount_factors: List[Decimal]) -> List[Decimal]:
        """Calculate present value of projected cash flows"""
        return [cf * df for cf, df in zip(cash_flows, discount_factors)]
    
    def _calculate_confidence_score(self, inputs: DCFInputs, graph_rag_context: Optional[Dict]) -> Decimal:
        """
    ...
    def _calculate_sensitivity_ranges(self, inputs: DCFInputs, wacc: Decimal) -> Dict[str, Tuple[Decimal, Decimal]]:
        """Calculate sensitivity ranges for key parameters"""
        return {
            "wacc": (wacc * Decimal("0.8"), wacc * Decimal("1.2")),
            "terminal_growth": (inputs.terminal_growth_rate * Decimal("0.5"), 
                              inputs.terminal_growth_rate * Decimal("1.5")),
            "base_growth": (inputs.growth_rates[0] * Decimal("0.7"),
