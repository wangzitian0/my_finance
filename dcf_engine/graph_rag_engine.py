...
from common.graph_rag_schema import (
    ...
    CYPHER_TEMPLATES, MAGNIFICENT_7_TICKERS, DocumentType
)
...
**Sources**: {self._format_sources(context.semantic_results)}
    ...
    def _extract_relevant_content(self, semantic_results: List[SemanticSearchResult]) -> str:
        ...
        for i, result in enumerate(top_results, 1):
            ...
            content_parts.append(f"**Source {i}** (Similarity: {result.similarity_score:.2f}):\n{content}")
    ...
    def _extract_risk_content(self, semantic_results: List[SemanticSearchResult]) -> Dict[str, str]:
        ...
        for result in semantic_results[:5]:  # Top 5 results
            ...
            if any(keyword in content for keyword in ['risk', 'uncertainty', 'volatility', 'challenge']):
                risk_factors.append(result.content[:200] + "..." if len(result.content) > 200 else result.content)
    ...
    def _analyze_investment_context(self, context: AnswerContext) -> Dict[str, str]:
        ...
        for result in context.semantic_results:
            ...
            # Positive indicators
            if any(word in content_lower for word in ['growth', 'profit', 'revenue increase', 'strong performance']):
                positive_signals += 1
    ...
    def _extract_dcf_assumptions(self, semantic_results: List[SemanticSearchResult]) -> str:
        ...
        if not assumptions:
            assumptions = ["• Standard DCF assumptions apply", "• Market-based discount rate", "• Conservative growth estimates"]
    ...
    def _generate_reasoning_steps(self, context: AnswerContext) -> List[str]:
        ...
        if context.semantic_results:
            steps.append(f"4. Found {len(context.semantic_results)} relevant documents via semantic search")
