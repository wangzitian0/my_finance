...
Provide a 2-sentence investment insight: (1) key strength or concern, (2) investment implication."""
            ...
            if result['success']:
                logger.info(f"✅ Generated LLM insight for {ticker} ({result.get('duration_seconds', 0):.1f}s)")
                return result['response'].strip()
            else:
                logger.warning(f"⚠️ LLM insight failed for {ticker}: {result.get('error', 'Unknown')}")
                return None
    ...
    def _merge_reports(self, traditional_report: str, llm_insights: Dict[str, str]) -> str:
        ...
        for line in lines:
            ...
            if (current_ticker and 
                current_ticker in llm_insights and 
                line.startswith("5-YEAR FREE CASH FLOW PROJECTIONS:")):
