...
class M7DCFAnalyzer:
    ...
    def calculate_wacc(self, metrics: Dict) -> float:
        ...
        debt_to_equity = metrics.get('debt_to_equity', 0) / 100 if metrics.get('debt_to_equity') else 0
    ...
    def calculate_dcf_valuation(self, metrics: Dict) -> Dict:
        ...
        upside_downside = ((intrinsic_value_per_share - current_price) / current_price * 100) if current_price > 0 else 0
    ...
    def generate_report(self) -> str:
        ...
        report_lines.append(f"Buy Recommendations: {len(recommendations['BUY'])} ({', '.join(recommendations['BUY'])})")
        report_lines.append(f"Hold Recommendations: {len(recommendations['HOLD'])} ({', '.join(recommendations['HOLD'])})")
        report_lines.append(f"Sell Recommendations: {len(recommendations['SELL'])} ({', '.join(recommendations['SELL'])})")
        report_lines.append("")
        ...
        for ticker in sorted(analyses.keys()):
            ...
            report_lines.append(f"Recommendation: {self.generate_investment_recommendation(analysis)}")
            report_lines.append("")
            ...
            report_lines.append(f"Free Cash Flow: {self.format_currency(metrics['free_cash_flow'])}")
            report_lines.append("")
            ...
            report_lines.append(f"Operating Margin: {self.format_percentage(metrics['operating_margin'])}")
            report_lines.append(f"Profit Margin: {self.format_percentage(metrics['profit_margin'])}")
            report_lines.append(f"ROE: {self.format_percentage(metrics['roe'])}")
            report_lines.append(f"Revenue Growth: {self.format_percentage(metrics['revenue_growth'])}")
            report_lines.append("")
            ...
            report_lines.append(f"Enterprise Value: {self.format_currency(dcf['enterprise_value'])}")
