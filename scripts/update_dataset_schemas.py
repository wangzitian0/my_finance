...
def validate_schema(config: Dict[str, Any], config_file: str) -> Dict[str, Any]:
    ...
    if "companies" in config:
        ...
        sector_coverage = (
            companies_with_sector / total_companies * 100 if total_companies > 0 else 0
        )

        if cik_coverage < 10:  # Less than 10% have CIK
            issues.append(
                f"Low CIK coverage: {cik_coverage:.1f}% ({companies_with_cik}/{total_companies})"
            )

        if sector_coverage < 50:  # Less than 50% have sector
            issues.append(
                f"Low sector coverage: {sector_coverage:.1f}% ({companies_with_sector}/{total_companies})"
            )
...
def main():
    ...
    for config_file in config_files:
        ...
        try:
            ...
            print(
                f"   CIK Coverage: {validation_result['companies_with_cik']}/{validation_result['companies_total']} ({validation_result['companies_with_cik']/max(validation_result['companies_total'],1)*100:.1f}%)"
            )
            print(
                f"   Sector Coverage: {validation_result['companies_with_sector']}/{validation_result['companies_total']} ({validation_result['companies_with_sector']/max(validation_result['companies_total'],1)*100:.1f}%)"
            )
            print(
                f"   Data Source Info: {'✅' if validation_result['has_data_source_info'] else '❌'}"
            )
