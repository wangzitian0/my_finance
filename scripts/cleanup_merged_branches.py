...
class BranchCleanup:
    ...
    def is_branch_merged(self, branch: str, target_branch: str = 'main') -> bool:
        ...
        try:
            ...
            result = subprocess.run([
                'git', 'merge-base', '--is-ancestor', branch, f'origin/{target_branch}'
    ...
    def cleanup_merged_branches(self, days_back: int = 30, force_local: bool = False) -> Dict[str, int]:
        """
        ...
        logger.info(f"Found {len(merged_branches)} branches from merged PRs in last {days_back} days")
    ...
    def interactive_cleanup(self) -> None:
        ...
        local_to_delete = [b for b in local_branches 
