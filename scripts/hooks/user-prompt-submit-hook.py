...
def parse_arguments():
    ...
    parser.add_argument(
        "--session-id",
        type=str,
        help="Override session ID (defaults to environment variable)"
    )
    
    parser.add_argument(
        "--user-agent",
        type=str,
        help="User agent information"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="claude_code",
        help="Source of the interaction"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=50000,
        help="Maximum prompt length to capture"
    )
...
def main():
    ...
    try:
        ...
        if len(prompt) > args.max_length:
            prompt = prompt[:args.max_length] + f"... [TRUNCATED - original length: {len(args.prompt)}]"
            if args.verbose:
