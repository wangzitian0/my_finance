...
def parse_arguments():
    ...
    parser.add_argument(
        "--response",
        type=str,
        help="Final AI response content"
    )
    
    parser.add_argument(
        "--thinking",
        type=str,
        help="AI thinking process content"
    )
    
    parser.add_argument(
        "--generation-time",
        type=int,
        help="Response generation time in milliseconds"
    )
...
def truncate_content(content: str, max_length: int, content_type: str = "content") -> str:
    ...
    if len(content) > max_length:
        truncated = content[:max_length] + f"... [TRUNCATED - original {content_type} length: {len(content)}]"
        return truncated
    return content
...
def main():
    ...
    try:
        ...
        context.update({
            ...
            "thinking_source": "thinking_file" if args.thinking_file else ("command_line" if args.thinking else None),
            "truncation_applied": {
                "response": len(args.response or response_content) > args.max_response_length,
                "thinking": thinking_content and len(thinking_content) > args.max_thinking_length
            }
        })
