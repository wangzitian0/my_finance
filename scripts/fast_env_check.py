...
def check_podman_machine():
    ...
    try:
        result = subprocess.run(['podman', 'machine', 'list'], 
...
def check_pandas_import():
    ...
    try:
        result = subprocess.run(['pixi', 'run', 'python', '-c', 'import pandas; print("OK")'], 
                              capture_output=True, text=True, timeout=2)
...
def main():
    ...
    try:
        with ThreadPoolExecutor(max_workers=3) as executor:
            # Submit all checks
            future_to_check = {executor.submit(check_func): name 
