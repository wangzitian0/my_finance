...
class ReleaseManager:
    ...
    def generate_release_manifest(self, artifacts: Dict[str, List[Path]], 
                                  release_id: str, build_path: Path) -> Dict:
        ...
        manifest["statistics"]["total_size"] = sum(stat["total_size"] for stat in manifest["statistics"].values() if isinstance(stat, dict))
    ...
    def create_release_package(self, artifacts: Dict[str, List[Path]], 
                              manifest: Dict, release_id: str) -> Path:
...
"""
    ...
    def create_release(self, release_name: Optional[str] = None, 
                      build_path: Optional[Path] = None) -> Tuple[str, Dict]:
        ...
        print(f"📊 Statistics: {manifest['statistics']['total_files']} files, {manifest['statistics']['total_size']:,} bytes")
    ...
    def list_releases(self) -> List[str]:
        ...
        release_dirs = [d.name.replace('release_', '') for d in releases_dir.iterdir() 
            ...
            for category, expected_count in manifest['validation']['file_counts'].items():
                ...
                if category_dir.exists():
                    ...
                    if actual_count != expected_count:
                        print(f"❌ File count mismatch for {category}: expected {expected_count}, found {actual_count}")
                        return False
