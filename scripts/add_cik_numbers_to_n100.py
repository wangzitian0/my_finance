...
def lookup_cik_for_ticker(ticker: str) -> Optional[str]:
    ...
    try:
        ...
        headers = {"User-Agent": "ZitianSG wangzitian0@gmail.com"}
...
def update_n100_config_with_ciks():
    ...
    if updated_count > 0:
        ...
        with open(config_path, "w", encoding="utf-8") as f:
            yaml.dump(
                config, f, default_flow_style=False, sort_keys=False, allow_unicode=True, width=1000
            )
...
def create_n100_sec_edgar_config():
    ...
    if companies_without_cik:
        print(
            f"   Missing CIK for: {', '.join(companies_without_cik[:10])}{'...' if len(companies_without_cik) > 10 else ''}"
        )
