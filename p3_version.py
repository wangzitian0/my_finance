...
class P3VersionManager:
    ...
    def get_version_info(self) -> Dict:
        ...
        if current_hash != self._version_data.get(
            "git_hash"
        ) or current_branch != self._version_data.get("git_branch"):
                ...
                for line in result.stdout.strip().split("\n"):
                    if line:
                        ...
                        if len(parts) == 2:
                            ...
                            history.append({"hash": commit_hash, "message": message})
...
def print_version_info():
    ...
    if git_dir:
        ...
        if post_merge_hook.exists():
            ...
            print(
                "⚠️  Git hooks not installed - run 'p3 install-version-hooks' for automatic updates"
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if command == "increment":
            ...
            print(
                "Usage: python p3_version.py [increment|update-on-pull|info|check-updates|history] [major|minor|patch|build]"
            )
    else:
        print_version_info()
